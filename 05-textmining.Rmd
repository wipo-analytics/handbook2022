...
In the previous chapter we worked with the PatentsView IPC table. The raw table contains a 'patent_id' field that we can link with other tables and a set of columns with different ipc levels that we can use to construct the four character subclass (e.g. C12N) with. We will use this table below with the patent_id field as the basis for joining with the titles and abstracts.

```{r eval=FALSE}
# read across from the connection
# fix errors on some documents where the ipc_class 
# does not contain a leading zero
...
When we have processed the text we will see that out 7.9 million individual document titles have expanded to 60,839,863 rows containing words in documents as we see in Figure \@ref(fig:titles)  which shows two titles. The important point here is that we know exactly what words appear in each patent document which is a very powerful tool. 
...
As we can see in Figure \@ref(fig:titles) there are many common words such as "and" that do not contain useful information. We can see the impact of these terms if we count up the words as we see in Figure \@ref(fig:commonwords).  
...
In Figure \@ref(fig:commonwords) we can see that common words will rise to the top but do not convey useful information. For this reason a common approach in text mining is to remove so called 'stop words'. Exactly what counts a stop word may vary depending on the task at hand. However, certain words such as `a, and, the, for, with` and so on are not useful if we want to understand what a text or set of texts is about. 

In tidytext there is a built in table of stop words and lists of stop words can be found on the internet that you can readily edit to meet your needs. We can see some of the stop words from tidytext in Figure \@ref(fig:stopwords). In reality tidytext includes three lexicons of stop words (onix, SMART and snowball) that you can use or adapt for your needs. There are also options to add your own.
...
```{r eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Tidytext Stop Words"}
# load data
...
Now when we count up the words in the grants titles we should get some more useful results as we see in Figure \@ref(fig:cleanwords). 
...
There are a variety of tools out there for lemmatizing text. We will use the textstem package by Tyler Rinker [@ref] in R which provides a range of options for lemmatizing words. In the code below we will create a new column with the lemmatized version of words called lemma. We will filter out any digits at the same time. Here we lemmatize the words that we have already applied stop words to. 

```{r lemmas, message=FALSE, echo=TRUE, eval=FALSE}
library(textstem)
# uses large grant words clean set
...
If we count up our lemmatized words we will see a dramatic change in the scores for terms such as 'method' and 'process' as we see in Figure \@ref(fig:lemmaout).
...
```{r eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Outcome of Lemmatization on Words in Patent Titles"}
# load head
load("data/text_mining/grant_lemma_head.rda")
# THIS DATA IS NOT FROM GRANT WORDS CLEAN AS INCLUDES STOP WORDS!
grant_lemma_head %>% 
  knitr::kable()
```
...
- Plant agriculture (AO1H), 
- Medicines and pharmaceuticals (A61K), 
- Biotechnology (C12N)
- Computing (G06F)

To do this we will start by identifying the patent_ids that fall into these subclasses. 
...
```{r eval=TRUE, echo=FALSE, fig.align='center', fig.cap="IPC Subclasses for TF_IDF"}
# load data
load("data/text_mining/ipc_set.rda")
...
We now join the two tables by the patent_id using `inner_join`. In SQL (here written with dplyr in R) an inner_join is a filtering join that will keep only shared elements in the two tables (the patent_ids). We then count the words and sort them.
...
```{r eval=TRUE, echo=FALSE, fig.align='center', fig.cap="HEAD OF IPC WORDS"}
# load head
...
Next we join the two tables together using left_join to keep everything in our original ipc_words table (that we will be applying the TFIDF to).
...
We are now in a position to apply the term frequency inverse document frequency calculations (using bind_tf_idf) as we see in  Figure \@ref(fig:tfid). 
...
```{r tfidplot, echo=TRUE, eval=FALSE}
...
ipc_words_tfid %>% 
  ...
  facet_wrap(ipc_subclass ~., labeller = as_labeller(ipc_names), ncol = 2, scales = "free") +
  coord_flip()
           
```

```{r echo=FALSE, fig.align='center', fig.cap="TFIDF scores for distinctive terms in patent titles by IPC subclass"}
...
The search terms are presented in Figure \@ref(fig:biodiversityipcwords). As these are search terms that are designed to be used in a search engine they include plurals and are not stemmed to avoid capturing many irrelevant terms. 

```{r biodiversityipcwords, eval=FALSE}
...
For this example we will scale up to include words from both the titles and abstracts of US granted patents. We will do this by uniting the titles and the abstracts into one string and then breaking that into words. Note that this is RAM intensive and if working on a machine with limited RAM you may wish to filter the grants data to a specific year to try this out. As an alternative, it is now very cost effective to create a powerful virtual machine with a cloud computing service (Amazon, Google, Microsoft etc.) that can be destroyed when a task is completed. The use of cloud computing is beyond the scope of this Handbook but is extremely useful for working with data at scale using either VantagePoint (on Windows Server) or with programming languages such as R, Python, Spark or Google Big Query. Many guides to getting started with creating virtual machines are available online and packages exist in R and Python that allow you to create and manage virtual machines from your desktop. Other options include Remote Desktop (for accessing remote virtual Windows machines as if they are your local desktop). 
...
```{r loadbiodiversitywords, echo=FALSE, eval=TRUE}
...
This will produce a dataset with our biodiversity terms and 2084 IPC subclasses. Figure \@ref(fig:ipcrank) shows the top 20 subclasses. 

```{r ipcrank, echo=TRUE, eval=TRUE, fig.align='center', fig.cap="Top 20 IPC Subclasses for Biodiversity Terms"}
...
```{r echo=TRUE, eval=FALSE}
biodiversity_ipc_subclass <- tibble(ipc_subclass = c("A01H","A01K","A01N","A23L","A23K","A23G","A23C","A61K","A61Q",
...
```{r loadbiodiversitypatentids, echo=FALSE, eval=TRUE, fig.align='center', fig.cap="Joining biodiversity words filtered by IPC subclasses to identify the original texts"}
...
```{r loadbiodiversitytexts, echo=FALSE, eval=TRUE}
load("data/text_mining/biodiversity_texts.rda")
```

```{r create_biodiversitybigrams, echo=FALSE, eval=TRUE}
...
```{r loadbiodiversitybigrams, echo=FALSE, eval=TRUE}
# DONT RUN
# save data
...
Figure \@ref(fig:head_biodiversitybigrams) shows a selection of the bigrams. 

```{r head_biodiversitybigrams, echo=FALSE, eval=TRUE, fig.align='center', fig.cap="Biodiversity Bigrams"}
# load data
...
```{r loadcleanbiodiversitybigrams}
# load 
...
As before this will radically reduce the size of the dataset to 9,321,285 although the set may still contain many irrelevant phrases. 

```{r biodiversitybigramhead, echo=FALSE, eval=TRUE, fig.align='center', fig.cap="Biodiversity Bigram Terms"}
...
We now have a set of bigrams with many irrelevant phrases. We could simply filter these phrases for terms of interest. In the chapter on patent citation analysis we focus on genome editing technology which is closely linked to genome engineering and synthetic biology. The extraction of bigrams allows us to identify patent grants containing these terms of interest in the title or abstracts.
...
```{r showgeneeditingbigrams, echo=FALSE, eval=TRUE, fig.align='center', fig.cap="Genome Editing Bigrams"}
...
However, we are still dealing with 9.5 million terms. Can we make this more manageable? The answer is yes. We can apply the tf_idf calculations we used for unigrams above to the bigrams. In this case we are applying the tf_idf calculation to the 345,975 patent grants in the biodiversity set (rather than IPC subclasses) to ask the question: What bigrams are distinctive to each document? As with the previous example, the code provided by Silge and Robinson  in _Text Mining with R_ is straightforward. 
...
The cooccurrence value for the matching term (in this case "gene editing") is always 1. Inside the actual matrix this displays on the diagonal in the centre of the matrix and gives rise to the expression "removing the diagonal" or self-reference. We can do this by filtering to keep any value that does not equal 1 as we see in Figure \@ref(fig:coocclean). 
...
In common with the network visualisations discussed in the last chapter, this data can also be visualised in a network where term1 constitutes the source node, term2 the target node and cooc (or n) the weight of the edge between source and target nodes. For simplicity in presentation we will select only terms with a cooccurence score over 30 for genome editing. 
...
```{r genet, echo=FALSE, fig.align='center', fig.cap="Genome Engineering Network"}
knitr::include_graphics("images/textmining/genomeediting.png")
```


The visualisation of networks is a powerful tool for making decisions about how to proceed with analysis. In particular, visualisation of networks of terms is an extremely useful device when seeking directions for further analysis, for example on the use of genome editing in therapeutics or in agriculture and other applications. A more thorough exploration than that provided here would have taken the combined set of terms to 
